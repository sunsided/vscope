/*==========================================================================*
	...
	Dateiname:		flageffect.cpp
	Beschreibung:	FlagEffect
	Revision:		1
...
void FLAGEFFECT::ProcessFlag() {
	...
	if (FAILED(hr = this->m_pInputTex->LockRect(0, &rect_src, NULL, D3DLOCK_READONLY))) {
		dprintf("(FEHLER) FLAGEFFECT internal: Flagge erstellen. Fehler %X während Textur-Lock 1.\n", hr);
		return;
	}

	if (FAILED(hr = m_pFlagTex->LockRect(0, &rect_dest, NULL, 0))) {
		dprintf("(FEHLER) FLAGEFFECT internal: Flagge erstellen. Fehler %X während Textur-Lock 2.\n", hr);
		return;
	}
	...
	if (FAILED(hr = this->m_pInputTex->UnlockRect(0))) {
		dprintf("(FEHLER) FLAGEFFECT internal: Flagge erstellen. Fehler %X während Textur-Unlock 1.\n", hr);
		return;
	}

	if (FAILED(hr = m_pFlagTex->UnlockRect(0))) {
		dprintf("(FEHLER) FLAGEFFECT internal: Flagge erstellen. Fehler %X während Textur-Unlock 2.\n", hr);
		return;
	}
